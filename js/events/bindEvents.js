function bindEvents({ state, elements, storage, defaultTotal }) {
  ...
  elements.initBtn.onclick = function () {
    const { resetStoredSettings } = window.SuksukApp.settingsStorage
    const { initGame } = window.SuksukApp.initGame
    const { resetCurrentSelection } = window.SuksukApp.picking

    resetStoredSettings(storage)
    initGame({ state, elements, storage, defaultTotal, resetCurrentSelection })
  }
  ...
  elements.ballContainerDiv.addEventListener("click", function (event) {
    ...
    const { returnPickedBall } = window.SuksukApp.picking
    returnPickedBall(clickedBallNumber, state, elements)
  })
}

function handlePickButtonClick(state, elements, storage, defaultTotal) {
  const { picking, finishLastBall, restartPicking } = window.SuksukApp.picking
...
function submitSettingsForm(event, state, elements, storage) {
  const { saveSettings } = window.SuksukApp.settingsStorage
...
function getSettingsFormValues(elements) {
  const { parseExcludedNumbers } = window.SuksukApp.initGame
